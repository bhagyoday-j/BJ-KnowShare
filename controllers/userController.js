...
//   const website_logo = req.file?.filename; //req.file?.path
...
//         const logoPath = `./public/uploads/${setting.website_logo}`;//req.file.path
...
const saveSettings = async (req, res, next) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      if (setting.website_logo_public_id) {
        await cloudinary.uploader.destroy(setting.website_logo_public_id);
      }

      // save new
      setting.website_logo = req.file.path;            // https cloud url
      setting.website_logo_public_id = req.file.filename; // cloud public id
    }
